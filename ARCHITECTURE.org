...
|---------|----------------|
| Colors | Semantic tokens: =--color-primary= (bright gold), =--color-accent= (deep gold), =--color-foreground=, =--color-muted=, =--color-destructive=, etc. |
| Typography | Inter (sans) for body, Lora (serif) for headings; weight 300 body, 600 headings |
| Logo | Black themed: dot is black, RENDER text is gray (#6b6b78), TENDER text uses foreground color |
...
| Progress | HealthBar renders rounded-full progress bar with =bg-muted-foreground/20= track background |
...
├── contexts/                 # React contexts (ToastContext, SidebarContext)
...
└── TenderDetailPanel (when tender selected)
    ...
    ├── TenderDetailTabs (tab navigation with action buttons)
    └── Tab Content:
        ├── TenderOpportunityTab (AI assessment, metadata, contact)
        ├── TenderAISummaryTab (AI-generated summary)
...
|-----|---------|-----------|
| Opportunity | AI quality assessment, metadata, contact | TenderOpportunityTab |
| AI Summary | AI-generated tender summary | TenderAISummaryTab |
...
Tab state managed locally in TenderDetailPanel. Action buttons (Shortlist, Review, Watch, Decline) appear in the tab navigation row.
...
| [[file:src/lib/quick_reference.org][src/lib/quick_reference.org]] | Lib, types, global contexts, pages details |
| [[file:src/pages/quick_reference.org][src/pages/quick_reference.org]] | Route-level page components, dependencies, patterns |
