...
| Styling | Tailwind CSS 4 + Arcade Design System | Utility-first styles with retro arcade aesthetic |
| Components | Custom Arcade UI | PixelBox, LEDNumber, HealthBar, ArcadeButton primitives |
...
Retro arcade-inspired visual language with light mode cream backgrounds and bold colored accents.

| Element | Implementation |
|---------|----------------|
| Containers | PixelBox with 4px offset shadows and pixel corner accents |
| Numbers | LEDNumber with glowing text shadow effect |
| Progress | HealthBar with segmented display |
| Buttons | ArcadeButton with gradient backgrounds and 3D press effect |
| Effects | Scanlines overlay, PixelStrip dividers |
| Colors | Cream (#FFFBF0) backgrounds, teal (#2D8F8F) and coral (#FF6B6B) accents |

Badges use arcade-styled variants: hexagon shapes for excellent fit, LED dots for status, gradient fills for importance.

See =src/components/quick_reference.org= for arcade component implementation details.
...
│   ├── shared/               # Arcade-styled Badges, States, Pagination
│   └── ui/                   # Arcade primitives (PixelBox, LEDNumber, etc.)
...
| AuthProvider | Authentication state | user, clientCode, isAuthenticated, login/logout |
...
| TenderCarousel | get_tenders_paginated → relevant tenders (limit: 8), toggles between carousel and grid views |
...
│  bar   │  │                         │                                    │ │
│        │  │ Filters:                │  Title + Recommendation Badge      │ │
│ ┌────┐ │  │ [Relevance ▼] [Search]  │  ─────────────────────────────     │ │
│ │Dash│ │  │                         │                                    │ │
│ └────┘ │  │ ┌─────────────────────┐ │  Key Info Cards:                   │ │
│ ┌────┐ │  │ │ ● Tender 1     [EX] │ │  ┌──────────┐ ┌──────────┐ ┌─────┐ │ │
│ │Tend│ │  │ │   Health • 5d       │◄──│ │ Closes   │ │ Briefing │ │ 87% │ │ │
│ └────┘ │  │ └─────────────────────┘ │  │ │ 5 days   │ │ Feb 3 !  │ │score│ │ │
│ ┌────┐ │  │ ┌─────────────────────┐ │  │ └──────────┘ └──────────┘ └─────┘ │ │
│ │Rubr│ │  │ │   Tender 2     [GD] │ │  │                                   │ │
│ └────┘ │  │ │   Treasury • 12d    │ │  │ Tabs: [Opportunity] [Evaluation]  │ │
│        │  │ └─────────────────────┘ │  │       [Documents] [Changes]       │ │
│        │  │                         │  │                                   │ │
│        │  │ < 1 2 3 ... 10 >        │  │ (Tab content below)               │ │
│        │  └─────────────────────────┴────────────────────────────────────┘ │
└────────┴───────────────────────────────────────────────────────────────────┘
#+END_EXAMPLE
...
└── TenderDetailPanel (when tender selected)
    ├── TenderDetailHeader (title, recommendation, key info cards)
...
Managed in TenderListPanel component:

| Filter | Type | Default |
|--------|------|---------|
| isRelevant | boolean or null | null (all) |
| provinceId | number or null | null |
| departmentId | number or null | null |
| categoryId | number or null | null |
| searchText | string | "" |
| sortBy | string | "recommendation" |
...
│ [≡ burger] [TenderRender]                    [pragma badge]  │
├────────────┬─────────────────────────────────────────────────┤
│            │                                                 │
...
- Shows error icon, message, and optional retry callback
...
|-------|---------|-------------|
| excellent_fit | Strong match, high priority | Emerald |
| good_fit | Good match, worth pursuing | Blue |
| worth_reviewing | Potential fit, needs review | Amber |
| not_recommended | Does not match criteria | Slate |
...
| Level | Meaning | Badge Color |
|-------|---------|-------------|
| high | Affects bid strategy (closing date, status, compulsory briefing) | Red |
| medium | Logistics impact (location, contact info) | Amber |
| low | Informational (description edits) | Slate |

** Tender Status

| Status | Meaning | Badge Color |
|--------|---------|-------------|
| active | Open for bids | Green |
| awarded | Contract awarded | Blue |
| closed | Bidding closed | Slate |
| cancelled | Tender cancelled | Red |

* Authentication Strategy

** Current Implementation

AuthContext-based authentication with development bypass via =VITE_DEV_CLIENT_CODE=.

*** Development Mode

When =VITE_DEV_CLIENT_CODE= is set, the app auto-authenticates with a mock user:

#+BEGIN_EXAMPLE
// Dev bypass in AuthContext
if (devClientCode) {
  setUser({ id: 'dev-user-001', email: 'dev@tenderrender.local' });
...
Without =VITE_DEV_CLIENT_CODE=, unauthenticated users are redirected to =/login=. The login form accepts email/password and calls =auth.login()=.

*** Auth State

| State | Type | Purpose |
|-------|------|---------|
| user | User or null | Current user (id, email) |
...
Still scaffolding - real Supabase Auth integration planned:

1. User authenticates via Supabase Auth
2. App queries =user_profiles= to get =client_pk=
3. App fetches =client_code= from =clients= table
4. All RPC calls use that client_code
