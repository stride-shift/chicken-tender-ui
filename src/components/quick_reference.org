...
| Header | layout/Header.tsx:10 | Top navigation bar with arcade branding, client code, and logout |
...
- Client code indicator badge
- Uses useAuth hook for logout
- Uses useNavigate for redirect to /login
- Logout button (ArcadeButton danger variant) in right section
