...
- Logo component at size "lg" with STRIDESHIFT / TENDERRENDER branding
- Breadcrumb showing current page name with "/" separator
- Pill-shaped profile button displaying:
  - User avatar (initial letter in teal circle)
  - User display name
  - Current client name
  - Dropdown arrow
- Profile dropdown (modern rounded design) contains:
  - User display name and admin status (StrideShift only)
  - Client selector list with checkmark for active (StrideShift admin only)
