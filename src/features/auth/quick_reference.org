...
|-----------|------|---------|
| LoginCard | LoginCard.tsx:10 | Semantic login form with email/password and forgot-password link |
| ForgotPasswordModal | ForgotPasswordModal.tsx:9 | Modal for sending password reset email via Supabase |
| CombinedLogo | CombinedLogo.tsx:19 | Logo with StrideShift Global + TenderRender, pixel boot on load |
| PixelBootLoader | PixelBootLoader.tsx:14 | Animated spinner loader with pulsing text and crossfade |

* Contexts & Hooks

| Item | File | Purpose |
|------|------|---------|
| AuthProvider | AuthContext.tsx:40 | Provider with two-effect auth state management, deadlock-safe |
| useAuth | AuthContext.tsx:319 | Hook to access auth state and methods (login, logout, etc.) |
| isAbortError | AuthContext.tsx:19 | Detect AbortError in both DOMException and Supabase-wrapped forms |
| recoverSupabaseAuth | AuthContext.tsx:33 | Reset cached initializePromise and reinitialize auth client |
