...
| useRelevantTenders | hooks/useRelevantTenders.ts:22 | get_tenders_paginated | Fetches relevant tenders for carousel display |
...
| p_sort_by | 'recommendation' | Sort by recommendation score (best first) |
| p_sort_desc | false | Ascending order (best recommendations first) |
