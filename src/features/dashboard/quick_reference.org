...
| StatCard | components/StatCard.tsx:12 | Single metric card with label and semantic color |
...
| ScoreDistributionChart | components/ScoreDistributionChart.tsx:17 | Recharts pie chart with category distribution |
| DepartmentTreemap | components/DepartmentTreemap.tsx:55 | Recharts horizontal bar chart of tenders by dept |
...
| useFilterOptions | @/features/tenders/hooks/useFilterOptions.ts | get_filter_options | Fetches departments with counts for bar chart |
...
| Feature | Implementation |
|---------|----------------|
| Chart type | Recharts pie chart (PieChart) |
| Data source | useDashboardStats hook |
| Colors | Excellent Fit (green-500 #22c55e), Good Fit (blue-500 #3b82f6), Worth Reviewing (amber-500 #f59e0b) |
| Labels | Percentage text with dark fill, shown if >5% |
...
| Colors | 10-color palette (blue, green, amber, pink, violet, teal, orange, cyan, red, slate) |
| Labels | Department name (Y-axis), count at end of bar |
| Tooltip | Custom tooltip showing department name and tender count |
...
| value | number or string | Metric value in monospace font |
| featured | boolean (optional) | Deprecated prop (no longer used) |
| accentColor | string (optional) | Top border color (default: #3b82f6 blue) |
